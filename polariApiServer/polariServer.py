        ...
        for _pair in seed_pairs:
            ...
            for seed in seed_list:
                ...
                try:
                    ...
                    import traceback
                    traceback.print_exc()
        # pf-1: the logic between the parts of every seeded tensor tree
        # exists from boot — obligations generated by the rules, the
        # never-run claims checked once (cheap tiers; z3 within each
        # claim's budget). Bounded by the aggregate's worst case.
        if _feature_available('mathproofs') and (
                only_classes is None or 'MathClaim' in only_classes):
            try:
                from mathproofs.custom.boot import run_at_boot
                run_at_boot(self.manager,
                            log=lambda s: print(s, flush=True))
            except Exception as e:
                print(f'[MathProofsBoot] failed: {e}', flush=True)
