...
from managedImages import managedImage
...
class managedImage_testClass(unittest.TestCase):
    ...
    def setUp(self):
        self.defaultTestClass = managedImage()
        self.namedTestClass = managedImage('TestFile')
    ...
    def test_singleDefault(self):
        classJSON = getJSONforClass(definingFile='managedImages', className='managedImage', passedInstances=self.defaultTestClass )
        logging.info(classJSON)

    def test_singleNamed(self):
        classJSON = getJSONforClass(definingFile='managedImages', className='managedImage', passedInstances=self.namedTestClass)
        logging.info(classJSON)
