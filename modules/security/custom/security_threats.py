...
THREATS = [
    ...
    {'name': 'ssh-password-guess', 'view': 'network', 'title': 'Someone guesses an ssh password',
     'actor': 'internet', 'means': 'guess a password over ssh', 'target': 'sshd',
     'story': 'With PasswordAuthentication on, a weak password is all that stands between the internet and a shell.',
     'counter': {'actor': 'owner', 'group': 'a person with their ed25519 key, from the admin LAN', 'means': 'ssh with a key', 'target': 'sshd',
                 'chain': [('ufw', 'allowed', '22 from the admin LAN only (rendered)'), ('ssh-keys', 'allowed', 'the key is the identity; PasswordAuthentication no')],
                 'story': 'Keys only, from the operator\'s network: the guessable path does not exist. Tracked per device on the isle topology (SshCapability).'}},
