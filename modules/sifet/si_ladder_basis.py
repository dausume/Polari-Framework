...
`manufacturability` (D-lod4-1, his rulings 2026-09-26) says which kind:
`open` ONLY when a fully open-source option AND route exist; `available`
when a third-party closed-source vendor takes orders (named in the
reason); `proven-on-request` (designs under the open PDK WERE fabricated
— shuttles / a foundry run — so a third party might take a request;
sky130 until a standing vendor route is verified); `unproven`;
`not-available`. The bool = open or available.
