...
in the SAME report shape as cntfet.cnt_montecarlo so every fi-3
...
@consumers cntfet.cnt_device_viz._montecarlo (dispatch by class)
"""

import numpy as np

from cntfet.cnt_metrics import extract_metrics
from cntfet.cnt_montecarlo import _envelope, _quantiles, _score_population
...
def _sample(rng, rows, device, k):
    ...
    import types
    from sifet.si_device import params_from_rows
...
def monte_carlo(manager, device, sample_count=100, seed=1, criteria=None,
    ...
    from sifet.si_device import params_from_rows, resolve_components
