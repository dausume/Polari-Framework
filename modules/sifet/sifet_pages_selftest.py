...
and drives the cell library through a pair-aware p card.
    ...
    cd modules && PYTHONPATH=..:../polariApiServer \\
        python3 -m sifet.sifet_pages_selftest
"""
...
from sifet.custom.si_device import derive_si_device, get_row, metric_spec
...
def main():
    ...
    check('fg-2 (fet, not cntfet): per-device Si pages are GONE — '
          ...
          SEED_SI_SCORE_PAGES == [])
    from cntfet.custom.cnt_compare import generic_pages
    ...
    from cntfet.cnt_cell_library_basis import _pair_params, characterize_cells
    from cntfet.custom.cnt_cells import _cards
