"""
@module sifet.si_model

fp-2: silicon MOSFET physics -> the virtual-source parameter dict
`p` that cntfet.cnt_vs_model.vs_terminal_current consumes. The VS
...
Equation set (revision sifet-vs-si-r1; VS core = cntfet revision):
  ...
  Id       = cntfet.cnt_vs_model.vs_terminal_current (REUSED)
...
@consumers
  - sifet.si_device (derive + device_model adaptor)
...
def sce(lg_nm, lambda_nm, phi_f_v):
    ...
    cntfet.cnt_bandstructure.sce_parameters ([VS1] eq.(8)):
...
def provenance_for(p):
    ...
    return {
        'equation_revision': SI_EQUATION_REVISION,
        'vs_core': 'cntfet.cnt_vs_model.vs_terminal_current (reused)',
