"""
@cross-cutting
@module aquaponics.custom.water_level
@tags @xc:bindings
...
period (aquaponics.water_batch_basis, phase 10), decomposed into THREE
...
"water disappears" number:
  ...
  1. DRAINAGE — gravity through the pot's own output hole. Reuses
     aquaponics.custom.hydraulics.reservoir_model()/build_darcy_payload()
  ...
  2. EVAPORATION — from the standing water's own surface, driven by
     REAL vapour-pressure-deficit (aquaponics.custom.atmosphere_analysis.
  ...
  3. TRANSPIRATION — "the plant absorbing the water" — driven by the
     ...
     (aquaponics.plant_growth_normalized_basis.current_canopy_profile) and
...
@consumers
  - aquaponics.plant_growth_normalized_basis.transport_factor (a bound
...
def water_level_trajectory(manager, planting_name, hours,
    ...
    from aquaponics.custom.hydraulics import build_darcy_payload, reservoir_model
    from aquaponics.custom.atmosphere_analysis import atmosphere_state
    ...
    return {
        ...
        'note': 'drainage reuses aquaponics.custom.hydraulics.reservoir_model '
