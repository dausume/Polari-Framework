"""
@cross-cutting
@module aquaponics.plant_growth_normalized
@tags @xc:bindings
...
Two explicit stages (point 3 above):
  ...
  constrained_limits()    STAGE 2 — free_soil_constants() run through
                          ...
                          plant_morphology.morphology_analysis.
...
@consumers
  - aquaponics.plant_skeleton (the bone/vector graph generator — the
...
#: Same convention aquaponics.plant_growth_simplified's own per-part
...
#: REAL absorbed light — aquaponics.light_field, phase 8), applied as
...
def constrained_limits(manager, pot_name, plant_name):
    ...
    geometry. Reuses plant_morphology.morphology_analysis.
...
class PotPlanting(treeObject):
    ...
    def __init__(
        ...
        plant_name: str = '',
        planted_at: str = '',
        # Optional aquaponics.pot_system.PotSystemDefinition name — the
        ...
        # atmosphere/water/soil rows (aquaponics.plant_stress, phase 7)
...
def closed_form_logistic(g0, effective_rate, ceiling, dt_days):
    ...
    aquaponics.plant_growth_simplified (2026-07-15) reuses it
...
def transport_factor(growth, limits, light_absorption_by_part,
    ...
    driven Liebig's-Law factor from aquaponics.nutrient_uptake.
    ...
    standing-water fraction (aquaponics.water_level.
    ...
    Public (not a `_`-prefixed helper) — aquaponics.plant_stress or a
...
def advance_growth(manager, planting_name, dt_days,
    ...
    matrix equations... atmospheric and soil and water inputs"):
      ...
      2. Neither passed + PotPlanting.system_name set — PER-PART
         ...
         aquaponics.plant_stress.combined_stress_factor() (Liebig's
    ...
    # days," see aquaponics.water_batch's own docstring); no schedule
    ...
    # (aquaponics.nutrient_uptake), against whichever water source is
    ...
    # drainage + evaporation + transpiration, aquaponics.water_level).
    ...
    if system is not None and getattr(
            system, 'water_batch_schedule_name', ''):
        from aquaponics.water_level import current_water_level_fraction
