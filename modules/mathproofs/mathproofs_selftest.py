...
check('the seed pairs cover every class (+ the techtree rows of the knowledge tree when techtree is present); eight inference rules; eighteen standalone claims (5 pf-0 + 7 pf-1 + 3 theorems pf-2 + 1 lod-3d restatement + 2 lod-2c twin witnesses)', {c_.__name__ for c_ in MATHPROOFS_CLASSES} <= {p[0] for p in MATHPROOFS_SEED_PAIRS} and len(SEED_INFERENCE_RULES) == 8 and len(SEED_MATH_CLAIMS) == 18)
