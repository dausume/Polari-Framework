...
SEED_INFERENCE_RULES = [
    ...
    _R(name='decomposition-reconstructs', pattern='mapping:decomposition', obligation_kind='bound', checker_default='numeric',
       template_json=json.dumps({'and': [{'gt': [{'ref': 'TensorMapping:{m1}', 'path': ['reconstruction_error']}, 0]}, {'le': [{'ref': 'TensorMapping:{m1}', 'path': ['reconstruction_error']}, 0.05]}]}),
       rationale='a decomposition must SAY what it loses: a recorded reconstruction error, and within the bound (0.05); an unrecorded (0) error is refused, not passed'),
