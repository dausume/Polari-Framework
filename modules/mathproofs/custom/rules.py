...
def generate(manager, tree, make=None, save=True, run_cheap=True):
    ...
    for rule in sorted(_rows(manager, 'InferenceRule'), key=lambda r: str(getattr(r, 'name', ''))):
        ...
        for structure, names, refs in _matches(rule, struct):
            ...
            result = None
            # a settled, fresh obligation is not re-run on every generation (the boot pass runs generate every start — the
            # ProofRun table must not grow by nine rows a boot); conjectured / stale ones are checked
            settled = str(getattr(claim, 'proof_status', 'conjectured')) not in ('conjectured',) and not checkers.stale(manager, claim)
            if run_cheap and term and not settled:
