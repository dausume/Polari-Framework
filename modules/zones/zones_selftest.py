...
Run: python3 -m zones.zones_selftest
"""
...
def phase_prism(m):
    from zones.custom.zone_geometry import (estimate_prism, estimate_zone,
                                     point_distances)
    _add_zone(m, 'square')
...
def phase_calibration(m):
    from zones.custom.zone_geometry import calibrate_zone, estimate_prism
    _add_zone(m, 'cal')
...
def phase_hull(m):
    from zones.custom.zone_geometry import estimate_hull
...
def phase_planar(m):
    from zones.custom.zone_geometry import estimate_planar
    from zones.custom.zone_packing import pack_zone
...
def phase_packing(m):
    ...
    from zones.custom.zone_packing import pack_zone, site_summary
...
def phase_summaries(m):
    from zones.zone_basis import SiteDefinition
    from zones.custom.zone_packing import room_summary, site_summary
