...
- REFERENCE (cation, MR): straight off the digitized Maekawa glass
  ...
  pspp.q_distribution — bands and refusals ride along unchanged.
...
Q-species are structural MOTIFS (see pspp.material_structure) — these
summaries never replace an underlying network representation.

@consumers
  - pspp.pspp_api (/api/pspp/structure/groups)
  - pspp.structure_sampling (target fractions for ensemble samples)
  - pspp.selftest_structure_groups
"""

from pspp.material_structure import descriptors_for_state
from pspp.q_distribution import (
    _Q4_ARTIFACT_CAVEAT,
    glass_to_solution_q,
