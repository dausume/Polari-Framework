...
@consumers
  ...
  - cntfet.cnt_device_viz (curves 'signal-terms',
...
from cntfet.cnt_regimes import REGIME_KNOBS, regime_frame
...
# ── citations this module adds (TAG_CITATIONS style; the integrator
#    merges them into cnt_citations.TAG_CITATIONS) ──────────────────
...
#: The signal characteristic-equation registry (FET_TERMS style).
SIGNAL_TERMS = {
    'fet-gm-over-id': {
...
def _sterm(name, display, description, unit, positive, lo, hi,
    ...
    row['source'] = 'cntfet.cnt_taxonomy (fp-3)'
...
SEED_SIGNAL_SCORE_CONCEPTS = [{
    ...
    'description': 'Weighted mean of the ANALOG figures of merit at '
                   ...
                   'Vdd·vds_fraction (cntfet.cnt_taxonomy SIGNAL_TERMS '
...
#: For cnt_device_viz.device_curve_points: fn(id_fn, p, device,
#: manager, knobs) → long-form rows.
CURVE_BUILDERS = {
    'signal-terms': _build_signal,
    'optimization-radar': _build_radar,
}

#: Sanity: FET_TERMS and SIGNAL_TERMS must not collide (two concepts).
