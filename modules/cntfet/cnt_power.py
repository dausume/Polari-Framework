...
@consumers
  ...
  - cntfet.cnt_cell_library (leakage blocks)
  - cntfet.cnt_device_viz (CURVE_BUILDERS)
  - polariServer (PowerBudget / ScoreTerm / GraphDefinition seeds)
  - cntfet.selftest_power
"""
...
from cntfet.cnt_states import LN10, model_vt, subthreshold_swing_v
...
#: Citations local to this module (the integrator may lift them
#: into cnt_citations.TAG_CITATIONS).
POWER_CITATIONS = {
...
#: Explicit knobs — every payload echoes what it used.
POWER_KNOBS = {
    'vdd_v': 0.6,
...
def cell_power(manager, device_name, cell_key, drive=1, knobs=None,
    ...
    from cntfet.cnt_cell_library import liberty_cell_name
...
#: Priors (plan §2 decision 2 — knobs, edit the rows).
SEED_POWER_BUDGETS = [
...
def cell_leakage_state_rows(id_fn, p, device, manager=None,
    ...
    from cntfet.cnt_cell_library import CELL_LIBRARY, liberty_cell_name
