"""
@module cntfet.cnt_scoring_seed
...
@consumers
  ...
  - cntfet.custom.cnt_montecarlo (fi-3: per-sample scores → best/worst)
  - cntfet.cnt_device_viz_seed (curve 'score-terms')
...
def _term(name, display, description, unit, positive, lo, hi,
    ...
    return {
        ...
        'abstract_tags_json': json.dumps(
             ...
             *tags]),
        'source': 'cntfet.cnt_scoring_seed (fi-2)',
...
SEED_FET_SCORE_CONCEPTS = [{
    ...
    'description': 'Weighted mean of the FET figures of merit, each '
                   ...
                   'ideal (cntfet.cnt_scoring_seed FET_TERMS). Weights '
...
def _provenance(manager, kind, name):
    from cntfet.cnt_device_viz_seed import provenance
