...
@consumers
  - cntfet.cnt_api (GET /api/cntfet/device/{name}/transport — wired
    by the integrator)
  - cntfet.cnt_device_viz (CURVE_BUILDERS for the transport graphs)
  - cntfet.selftest_transport
...
from cntfet.cnt_bandstructure import M0_KG, m_eff_over_m0
from cntfet.cnt_constants import KB_J_PER_K, Q_C, lit_value
from cntfet.cnt_derive import get_row, resolve_components
from cntfet.cnt_device_viz import _device_graph
from cntfet.cnt_states import OPS, evaluate_criteria
...
# ── device context from the component + process rows ─────────────
...
def transport_context(manager, device):
    ...
    from cntfet.cnt_montecarlo import gather_process_set
...
# ── mechanism definitions (manager rows win) ──────────────────────
...
# ── long-form rows for the fv-2 graphs ────────────────────────────
