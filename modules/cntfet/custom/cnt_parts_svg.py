"""
@module cntfet.custom.cnt_parts_svg
...
def _cnt_layout(manager, device, parts, knobs):
    ...
    from cntfet.cnt_fields_basis import device_regions
    from cntfet.cnt_scene_seed import _sketch_geometry
...
def _overlay(manager, device, field, vg, vd):
    ...
    (fg-4: the Si sketch lives in sifet.custom.si_fields; its x axis uses
    the SAME parts2d sketch lengths so the overlay aligns)."""
    from cntfet.cnt_device_viz_seed import device_vdd
    from cntfet.cnt_fields_basis import FIELDS, field_profile
