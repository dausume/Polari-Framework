"""
@module cntfet.cnt_device_viz_seed
...
def device_model(manager, name):
    ...
    if device is None:
        ...
        try:
            from sifet.custom.si_device import si_device_model
...
def state_curve_rows(id_fn, p, curve, vd=0.6, manager=None, vdd=0.6):
    ...
    from cntfet.cnt_states_basis import output_boundary, state_band_rows
    rows = []
...
def _montecarlo(manager, device, samples, seed):
    ...
    if not hasattr(device, 'process_set'):
        ...
        try:
            from sifet.custom.si_montecarlo import monte_carlo as si_mc
...
def score_curve_rows(manager, id_fn, p, device, curve, vd=0.6,
    ...
    if curve == 'cell-scores':
        from cntfet.cnt_cell_scoring_seed import cell_score_rows, score_cells
        report = score_cells(manager, device.name)
        if not report.get('ok'):
            return None, report
        return cell_score_rows(report), None
    if curve == 'compare':
        from cntfet.custom.cnt_compare import compare_devices, compare_rows
...
def provenance(manager, subject_kind, name):
    ...
    rather than fake it."""
    try:
        from cntfet.cnt_evidence_basis import provenance_summary
    except ImportError:
        return None
    try:
