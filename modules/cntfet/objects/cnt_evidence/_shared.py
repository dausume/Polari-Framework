"""@module cntfet.objects.cnt_evidence._shared — what the cnt_evidence row classes share (constants, seeds, helpers); split from cnt_evidence_basis.py (sap-2c)."""
...
try:
    from sifet.custom.si_model import SI_CITATIONS
except ImportError:  # sifet optional at import time
    SI_CITATIONS = {}
try:
    from sifet.si_refinement_basis import CITATIONS as SI_REFINEMENT_CITATIONS
except ImportError:
...
SEED_EVIDENCE = [
    ...
    _pub('AP97', "Scaling theory for cylindrical, fully-depleted, "
         ...
         notes='also cited as [AP98] in sifet.custom.si_model'),
    ...
    _pub('NAR01', 'Scaling of stack effect and its application for '
         ...
         'model', verified=V_XR),
    # sifet.custom.si_model citations
    ...
    _pub('SG-SIO2', 'Open-literature sol-gel SiO2 (TEOS) spin-on '
         ...
         'with DOI + date'),
    # sifet.si_refinement_basis citations
...
def _library_proof_core(manager, today=None):
    ...
    if not route_names:
        try:
            from sifet.si_refinement_basis import SEED_REFINEMENT_ROUTES
...
def _intended_use_of(manager, subject_kind, name):
    ...
    if subject_kind == 'record':
        ...
        try:
            from cntfet.cnt_ip_basis import INTENDED_USE
