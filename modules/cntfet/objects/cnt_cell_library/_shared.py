"""@module cntfet.objects.cnt_cell_library._shared — what the cnt_cell_library row classes share (constants, seeds, helpers); split from cnt_cell_library_basis.py (sap-2c)."""
...
def _liberty_library(vdd, slews_s, loads_f, cell_blocks):
    ...
    out = [
        'library (polari_cnt) {\n'
        '  /* generated by cntfet.cnt_cell_library_basis — executor '
        'polari-own-loop.\n'
...
def characterize_cells(manager, device, cells=None, drives=(1,),
    ...
    try:
        from cntfet.cnt_power_basis import leakage_blocks_for
...
def library_report():
    ...
    from cntfet.custom.cnt_sequential import lctime_status
    return {'ok': True,
