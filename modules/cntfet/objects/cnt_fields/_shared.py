"""@module cntfet.objects.cnt_fields._shared — what the cnt_fields row classes share (constants, seeds, helpers); split from cnt_fields_basis.py (sap-2c)."""
...
def _analytic_ec(xs, x_edge, lam, c_const, efsd, vd):
    from cntfet.custom.kwant_worker import analytic_ec
...
def field_profile(manager, device, field, vg=None, vd=None, n=60,
    ...
    if dev is not None and not hasattr(dev, 'material'):
        ...
        # (sifet.custom.si_fields — same eq.(5) barrier, silicon λ); one
        # dispatch point so /fields and the parts2d overlays
        # un-refuse together. Absent module → the old refusal.
        try:
            from sifet.custom.si_fields import field_profile_si
        except ImportError:
            pass
        else:
...
SEED_FET_FIELD_MATERIALS_3D = [
    ...
    _mat('fet-gate-metal', '#ffb300', 'Gate metal shell (W prior)',
         opacity=0.45),
    # fg-6: silicon region materials (sifet.custom.si_scene box stacks)
