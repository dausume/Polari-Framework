"""@module motors.objects.clock_views._shared — what the clock_views row classes share (constants, seeds, helpers); split from clock_views_basis.py (sap-2c)."""
import json
...
SECTION_SOURCES = {
    ...
    'scale-study': lambda m, a: __import__(
        'motors.scale_goals_basis', fromlist=['scale_study']
    ).scale_study(m, a.get('policy', 'local-plus-imported-wire')),
    'goal-feasibility': lambda m, a: __import__(
        'motors.scale_goals_basis', fromlist=['goal_feasibility']
    ).goal_feasibility(m, a['goal']),
    # mechanical
    'failure-conditions': _failure_conditions,
    'load-cases': _src_design('motors.custom.motor_stress', 'load_cases'),
    'part-stress': _src_part('motors.custom.motor_stress', 'part_stress'),
    'part-fatigue': _src_part('motors.custom.motor_fatigue',
                              'part_fatigue'),
    'contact-stress': _src_part('motors.custom.contact_wear',
                                'contact_stress'),
    'stress-tensor-field': _stress_tensor_gap,
    # electrical (separable from magnetic, per Dustin)
    'winding': _src_design('motors.custom.motor_winding',
                           'winding_report'),
    'winding-gauge-sweep': _src_design('motors.custom.motor_winding',
                                       'gauge_sweep'),
    'drive-pulse-response': _src_design('motors.custom.inductance',
                                        'pulse_response'),
    # magnetic
    'inductance': _src_design('motors.custom.inductance',
                              'solve_inductance'),
    'model-validity': _src_design('motors.custom.inductance',
                                  'model_validity'),
    'torque-curve': _src_design('motors.custom.motor_designer',
                                'torque_curve'),
    # materials + sourcing + dependency trace
    'materials-accountability': _src_design(
        'motors.custom.motor_materials', 'material_accountability'),
    'composition-structure': _src_design(
        'motors.custom.composition_splice', 'composition_view'),
    'promotion-candidates': _src_design(
        'motors.custom.composition_splice', 'promotion_candidates'),
    # mass
    'mass-bill': _src_design('motors.motor_parts_basis', 'part_report'),
    # as-1/3: the genuine assembly + the end-to-end time proof.
    'clock-assembly': lambda m, a: __import__(
        'motors.clock_assembly_seed', fromlist=['clock_assembly_report']
    ).clock_assembly_report(m),
    'timekeeping-proof': lambda m, a: __import__(
        'motors.clock_assembly_seed', fromlist=['timekeeping_proof']
    ).timekeeping_proof(m, pulses=int(a.get('pulses', 120))),
    # mp0: the product, twice-sourced.
    'sourcing-routes': lambda m, a: __import__(
        'motors.product_routes_seed', fromlist=['product_routes']
    ).product_routes(m, design_name=a.get('design',
                                          'clock-lavet-m0b')),
    # bench-1: the W2 measurement protocol.
    'bench-campaign': lambda m, a: __import__(
        'motors.custom.bench_campaign', fromlist=['bench_campaign']
    ).bench_campaign(m, design_name=a.get('design',
                                          'clock-lavet-m0b')),
    # motion
    'clock-sim': _src_design('motors.custom.motor_designer', 'clock_sim'),
    'verification': _src_design('motors.custom.motor_verify',
                                'verification_summary'),
    # cost (provenance-driven; the accountability chain carries the
    # cascaded make-vs-buy trace)
    'lifecycle-cost': _src_part('motors.custom.lifecycle_cost',
                                'cost_per_lifespan_unit'),
    'cheapest-configuration': _src_part('motors.custom.lifecycle_cost',
...
def component_view(manager, part_name, design='clock-lavet-m0'):
    ...
    from motors.custom.lifecycle_cost import cost_per_lifespan_unit
    run('lifecycle-cost',
