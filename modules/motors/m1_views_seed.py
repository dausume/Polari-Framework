...
motors.m1_selftest, polariServer seed pass (seed_m1_views)
"""
...
def _m1_min_current(manager, a):
    ...
    from motors.custom.m1_sequencing import m1_minimum_drive_current
    load = a.get('load')
    basis = ''
    if load == 'axis-duty':
        from motors.m1_positioning_basis import axis_report
        from motors.m1_product_seed import GEAR_RATIO
...
M1_SECTION_SOURCES = {
    'm1-sequence': _src_m1('motors.custom.m1_sequencing', 'sequence_sim'),
    'm1-holding': _src_m1('motors.custom.m1_sequencing', 'holding_torque'),
    'm1-min-current': _m1_min_current,
    'm1-axis': _src_m1('motors.m1_positioning_basis', 'axis_report'),
    'drive-profile': _src_m1('motors.motor_drive_basis',
                             'simplefoc_config'),
    'm1-phase-electrics': _src_m1('motors.m1_views_seed',
                                  'm1_phase_electrics'),
    'role-screen': lambda m, a: __import__(
        'motors.custom.part_roles', fromlist=['screen_candidates']
    ).screen_candidates(m, a['part_name']),
    'm1-construction-fork': lambda m, a: __import__(
        'motors.m1_composition_seed', fromlist=['m1_construction_fork']
    ).m1_construction_fork(m),
    'm1-positioning-proof': _src_m1('motors.m1_positioning_basis',
                                    'positioning_proof'),
    # cons-3: the arc rules, live off the shape rows.
    'm1-arc-rule': lambda m, a: __import__(
        'motors.m1_relations_seed', fromlist=['arc_rule_report']
    ).arc_rule_report(m),
    'm1-overlap-adoption': lambda m, a: __import__(
        'motors.m1_relations_seed', fromlist=['overlap_model_gap']
    ).overlap_model_gap(m),
}
...
from motors.m1_scene_seed import m1_scene_json_for_view  # noqa: E402
