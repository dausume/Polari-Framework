"""
@module motors.custom.local_route
...
def route_power_effect(manager, design_name='clock-lavet-m0',
    ...
    from motors.custom.clock_product import (POWER_REFERENCES,
                                      power_budget, CELLS)
...
def turns_sweep(manager, design_name='clock-lavet-m0',
    ...
    from motors.custom.clock_product import POWER_REFERENCES, CELLS
    design = _named(manager, 'MotorDesignDefinition', design_name)
    if design is None:
        return {'ok': False,
                'refusal': f'no design named "{design_name}"'}
