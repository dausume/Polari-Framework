"""
@module motors.custom.composition_splice
...
from motors.custom.part_roles import PART_ROLE_ASSIGNMENTS
...
def interface_specs(design_name):
    ...
    if design_name == 'reluctance-6s4p-m1':
        from motors.m1_composition_seed import M1_INTERFACES
        return M1_INTERFACES
    if design_name == 'ferrite-pm-m2':
        from motors.m2_composition_seed import M2_INTERFACES
...
def _declared_level(design_name):
    if design_name == 'reluctance-6s4p-m1':
        from motors.m1_composition_seed import M1_DECLARED_LEVEL
        return M1_DECLARED_LEVEL
    if design_name == 'ferrite-pm-m2':
        from motors.m2_composition_seed import M2_DECLARED_LEVEL
...
def _promotion_answers(design_name):
    ...
    if design_name == 'reluctance-6s4p-m1':
        from motors.m1_composition_seed import M1_PROMOTION_ANSWERS
        return M1_PROMOTION_ANSWERS
    if design_name == 'ferrite-pm-m2':
        from motors.m2_composition_seed import M2_PROMOTION_ANSWERS
...
def _movement_parts(manager, design_name):
    ...
    from motors.motor_parts_basis import PART_REUSE
