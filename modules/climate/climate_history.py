"""
@module climate.climate_history
...
climate.co2_trend makes it from the instrumental growth-rate
...
climate.co2_trend, climate.selftest_co2, polariServer (seed)
"""
...
ICE_INGEST = ("climate.series_ingest.ingest_series(manager, "
...
def era_co2_summary(manager, series_name=ICE_SERIES):
    ...
    if not eras:
        return _refuse(
            ...
            'call climate.climate_history.seed_human_eras(manager)')
    out = []
    for era in eras:
...
def outside_experienced_range(manager, present_ppm,
    ...
    return {
        ...
        'note': (
            ...
            '(climate.co2_trend over the NOAA annual growth '
...
def coverage_citations(manager, series_name, from_year=None,
    ...
    if not spans:
        return _refuse(
            ...
            f'climate.series_ingest.ingest_series for this '
