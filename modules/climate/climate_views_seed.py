"""
@module climate.climate_views_seed
...
payload. The assembler is motors/clock_views_basis.py's `view_payload`
...
@consumers climate.climate_api, climate.custom.climate_export,
climate.climate_page, climate.climate_selftest
"""
...
from climate.custom.co2_trend import compare_velocity, velocity_from_source
...
INGEST_OUTDOOR = (
    "climate.custom.series_ingest.ingest_series(manager, "
...
def _indoor_coupling(manager, a):
    ...
    if not spaces:
        return _refuse(
            ...
            'climate.co2_indoor_seed.seed_indoor_spaces(manager), or '
...
def _crossings(manager, a):
    ...
    if not thresholds:
        return _refuse(
            ...
            'climate.co2_thresholds_seed.seed_co2_thresholds(manager)')
    spaces = rows(manager, 'IndoorSpaceProfile')
    if not spaces:
        return _refuse(
            'no IndoorSpaceProfile rows — run '
            'climate.co2_indoor_seed.seed_indoor_spaces(manager)')
...
def _series_replacement(row):
    ...
    return {
        ...
        'replacesWith': f'ingest it from {endpoint} '
                        f'(climate.custom.series_ingest.ingest_series)',
        'kind': 'series',
    }
...
def _biochemistry(manager, args):
    ...
    proxies."""
    from climate.custom.co2_biochemistry import biochemistry_report
    from composition.custom.data_refs import rows as _r
    ca, bp, cl, ag = {}, {}, {}, {}
