...
def fetch_kernel_table(release, base_name):
    ...
    try:
        ...
        mode = {'data.tar.xz': 'r:xz', 'data.tar.gz': 'r:gz', 'data.tar.zst': 'zstd', 'data.tar': 'r:'}.get(data[0], None)
        if mode is None:
            return {'kernel_table': '', 'kernel_abi': abi, 'kernel_note': f'{data[0]} compression not readable here'}
        blob = data[1]
        if mode == 'zstd':   # Ubuntu's debs are zstd-compressed: the zstd binary (in the image) inflates to a plain tar
            if not shutil.which('zstd'):
                return {'kernel_table': '', 'kernel_abi': abi, 'kernel_note': 'the kernel deb is zstd-compressed and zstd is not installed here'}
            r = subprocess.run(['zstd', '-dc'], input=blob, capture_output=True)
            if r.returncode != 0:
                return {'kernel_table': '', 'kernel_abi': abi, 'kernel_note': 'zstd could not inflate the kernel deb'}
            blob = r.stdout; mode = 'r:'
        # modules.alias is generated by depmod at install time, never shipped: derive it the way depmod does — from
        # every module's .modinfo 'alias=' strings (the .ko files ride zstd/xz-compressed in the deb)
        n_mod = 0; lines = []
        with tarfile.open(fileobj=io.BytesIO(blob), mode=mode) as t:
            for m in t.getmembers():
                ...
                if m.name.endswith('.zst'):
                    raw = subprocess.run(['zstd', '-dc'], input=raw, capture_output=True).stdout
                elif m.name.endswith('.xz'):
                    import lzma; raw = lzma.decompress(raw)
                elif m.name.endswith('.gz'):
                    raw = gzip.decompress(raw)
                modname = re.sub(r'\.ko(\.zst|\.xz|\.gz)?$', '', os.path.basename(m.name)).replace('-', '_')
                for al in module_aliases(raw):
                    lines.append(f'alias {al} {modname}')
                n_mod += 1
        if not lines:
            return {'kernel_table': '', 'kernel_abi': abi, 'kernel_note': f'no alias strings found in {n_mod} modules'}
        open(dest, 'w').write('\n'.join(lines) + '\n')
        return {'kernel_table': dest, 'kernel_abi': abi, 'kernel_note': f'{len(lines)} aliases from {n_mod} modules'}
    except Exception as exc:   # noqa: BLE001
        return {'kernel_table': '', 'kernel_abi': '', 'kernel_note': f'kernel table fetch failed: {type(exc).__name__}: {exc}'}


def module_aliases(ko_bytes):
    """The 'alias=' entries of a kernel module's .modinfo section (NUL-separated strings) — what depmod reads."""
    out = []
    for m in re.finditer(rb'alias=([^\x00]{1,200})\x00', ko_bytes):
        a = m.group(1).decode('ascii', 'ignore')
        if a.startswith(('pci:', 'usb:')):
            out.append(a)
    return out
