...
BINDING what already exists rather than building:
  ...
  chain_full_report       chain_report (parity + thermal) + the
                          ...
                          from supplychain.mold_analysis's
...
#: Stage mold material → mold_analysis strategy key. Feedstock waxes
#: all melt-reclaim; PLA has no strategy prior (named gap).
_STRATEGY_OF = {
...
def chain_full_report(manager, chain_name):
    ...
    try:
        from supplychain.mold_analysis import MOLD_STRATEGY_PRIORS
    except ImportError:
    ...
    return {'ok': True, 'chain': chain_name,
            ...
            'gaps': gaps,
            'note': 'economics ride supplychain.mold_analysis '
