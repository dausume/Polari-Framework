"""
@cross-cutting
@module casting.custom.voxel_grid
@tags @xc:bindings

cast-2: the framework's first REUSABLE occupancy grid. The voxelizer
has existed since shape-1 — buried inside
mathshapes.custom.shape_analysis._grid_properties, which builds an interior
...
@consumers
  - casting.custom.mold_geometry (grid derivation for imported meshes),
    casting.custom.mesh_voxelize, cast-5 fill sim, cast-7 demold sweep
...
class OccupancyGrid:
    ...
    def from_shape(cls, manager, shape_name, resolution=32,
        ...
        casting.custom.mesh_voxelize.mesh_grid is their honest path."""
        ...
        if family == 'imported-mesh':
            return {'ok': False,
                    'error': f"'{shape_name}' is an imported mesh — "
                             ...
                             f'casting.custom.mesh_voxelize.mesh_grid '
                             f'instead'}
