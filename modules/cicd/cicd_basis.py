...
#: dep-0: DeployTarget is a SETTINGS row too — where Polari runs and under what conditions the pipeline
#: may replace its images (never its secrets: the key there is restricted to the deploy agent).
CICD_SETTINGS_CLASSES = [PipelineDevice, PipelineStage, PipelineRoute, DeployTarget]
...
#: dep-1: DeployRecord — what runs where, since when, from which release, and how the deploy went.
CICD_MIRROR_CLASSES = [PipelineSecretPresence, PipelineRun, IsleTestResult, ReleaseRecord, PipelineSetupStep,
                       TestVerdict, DeployRecord]
