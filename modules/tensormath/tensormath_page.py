...
SEED_TENSORMATH_PAGE_DISPLAYS = [
    _page('tensormath', 'tensormath',
          ...
          'Tensor', [
              ...
              _row(2, [_table('tensormath-tensors', 0, 7, 'Tensors — unknown semantics is valid', 'Tensor', columns='name,rank,shape_json,dtype,dimensions_json,units,semantics,storage_kind,storage_ref'),
                       _table('tensormath-dimensions', 1, 5, 'Named dimensions', 'TensorDimension', columns='tensor,index,name,kind,unit,size,semantics')]),
              _row(3, [_table('tensormath-expressions', 0, 6, 'Expressions — rank ≤ 2 delegates to the matrix module', 'TensorMathExpression', columns='name,operation,latex,operands_json,dims_json,matrix_equation_ref,result_shape_json'),
