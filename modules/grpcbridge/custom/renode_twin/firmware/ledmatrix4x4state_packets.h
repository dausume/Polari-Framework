 ...
 * class: LedMatrix4x4State   contract v1   hash 66dd503db2502aee
 * Byte-exact twin of the Java LedMatrix4x4StateCodec / PolariPacket:
 * regenerate when the contract regenerates. */
#ifndef LEDMATRIX4X4STATE_PACKETS_H
#define LEDMATRIX4X4STATE_PACKETS_H

#ifndef POLARI_RX_PAYLOAD_MAX
#define POLARI_RX_PAYLOAD_MAX 156u
#endif
...
#define LEDMATRIX4X4STATE_MSG_TYPE 3u
#define LEDMATRIX4X4STATE_PAYLOAD_MAX 156u
...
static uint16_t LedMatrix4x4State_encode(const LedMatrix4x4State_t *s,
                                    uint8_t *p)
{
    uint8_t *start = p;
    uint16_t n;
    (void)n;
    n = (uint16_t)strlen(s->driver);
    polari_put_u16(p, n); p += 2;
    memcpy(p, s->driver, n); p += n;
    polari_put_u32(p, (uint32_t)s->height);
    polari_put_u32(p + 4, (uint32_t)((uint64_t)s->height >> 32)); p += 8;
    n = (uint16_t)strlen(s->name);
    polari_put_u16(p, n); p += 2;
    memcpy(p, s->name, n); p += n;
    polari_put_u32(p, (uint32_t)s->pixels);
    polari_put_u32(p + 4, (uint32_t)((uint64_t)s->pixels >> 32)); p += 8;
    polari_put_u32(p, (uint32_t)s->width);
    polari_put_u32(p + 4, (uint32_t)((uint64_t)s->width >> 32)); p += 8;
    return (uint16_t)(p - start);
}

/* payload bytes -> struct. Returns 0, or -1 on truncated input. */
static int LedMatrix4x4State_decode(const uint8_t *p, uint16_t len,
                               LedMatrix4x4State_t *s)
{
    ...
    memcpy(s->driver, p, cp); s->driver[cp] = 0; p += n;
    if (end - p < 8) return -1;
    s->height = (int64_t)((uint64_t)polari_get_u32(p)
    ...
    if (end - p < 8) return -1;
    s->pixels = (int64_t)((uint64_t)polari_get_u32(p)
        | ((uint64_t)polari_get_u32(p + 4) << 32)); p += 8;
    if (end - p < 8) return -1;
    s->width = (int64_t)((uint64_t)polari_get_u32(p)
        | ((uint64_t)polari_get_u32(p + 4) << 32)); p += 8;
    return 0;
}

#endif /* LEDMATRIX4X4STATE_PACKETS_H */
