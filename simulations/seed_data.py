...
# ==========================================================================
# NEWTONIAN PENDULUM (3D) — reality-first reformulation.
#
...
# vector/matrix calculus (MatrixEquationOperation no-code states). Gravity
# is a Partial contributing to the f_app_* accumulator; wind will be a
# sibling Partial later. The rod + two force arrows render off the bob via
# cross-class `depends_on`. Non-destructive: the Simple pendulum above is
...
from simulations.newtonian_pendulum_viz_states import (
    ...
    NewtonianPendulumGravityVectorSimState as _NGravCls,
    NewtonianPendulumNetVectorSimState as _NNetCls,
)
...
_NGV = 'NewtonianPendulumGravityVectorSimState'
_NNV = 'NewtonianPendulumNetVectorSimState'
...
# --- Viz companions: rod + two force arrows (read the bob's vectors via deps) ---
_NEWTON_ARROW_OUTPUT_MAP = {
    'src_x': _element_source('p_c', 0), 'src_y': _element_source('p_c', 1), 'src_z': _element_source('p_c', 2),
    'tip_x': _element_source('tip', 0), 'tip_y': _element_source('tip', 1), 'tip_z': _element_source('tip', 2),
}
...
_NB_GVEC_DEF, _NB_GVEC_META, _NB_GVEC_EQ = _step_solution_pair(
    name=f'{NEWTON_SIM_DEF}.gravity-vector.step',
    description='Gravity force arrow — from the bob to bob + viz_force_scale·F_grav.',
    target_class=_NGV, expected_inputs=['p_c', 'fgrav', 'viz_force_scale'],
    compute_steps=[_mat_step('GravTip', 'tip', 'newton-arrow-tip',
                             [('p', _from_source('self.p_c')),
                              ('k', _from_source('self.viz_force_scale')),
                              ('f', _from_source('self.fgrav'))],
                             'tip = bob + k·F_grav')],
    output_field_to_context=_NEWTON_ARROW_OUTPUT_MAP,
    sim_step_role='simStepComplete', order_index=0, depends_on=[_NB],
)
_NB_NVEC_DEF, _NB_NVEC_META, _NB_NVEC_EQ = _step_solution_pair(
    name=f'{NEWTON_SIM_DEF}.net-vector.step',
    description='Net force arrow — from the bob to bob + viz_force_scale·F_net.',
    target_class=_NNV, expected_inputs=['p_c', 'fnet', 'viz_force_scale'],
    compute_steps=[_mat_step('NetTip', 'tip', 'newton-arrow-tip',
                             [('p', _from_source('self.p_c')),
                              ('k', _from_source('self.viz_force_scale')),
                              ('f', _from_source('self.fnet'))],
                             'tip = bob + k·F_net')],
    output_field_to_context=_NEWTON_ARROW_OUTPUT_MAP,
    sim_step_role='simStepComplete', order_index=0, depends_on=[_NB],
)
...
SEED_NEWTON_GRAV_ROWS = [_newton_step0_row(_NGravCls, 'newtonian-pendulum-gravity-vector')]
SEED_NEWTON_NET_ROWS = [_newton_step0_row(_NNetCls, 'newtonian-pendulum-net-vector')]

_NEWTON_SIMSPACE = {
    'name': 'newtonian-pendulum-viz',
    'description': (
        '3D visualization of the Newtonian pendulum. The bob is a sphere sized '
        'to its metric radius; the rigid rod and the gravity (red) + net (green) '
        'force arrows are connections that track the bob each step. Energies are '
        'derived (KE/PE/E_total) and shown for conservation checking.'
    ),
    'dimensionality': '3d',
    'coordinate_system': 'math',
    'unit_scale': 1.0,
...
_NEWTON_BINDINGS = [
    ...
    {
        'name': f'{_NGV}-3d', 'class_name': _NGV, 'dimensionality': '3d', 'enabled': True,
        'binding_json': json.dumps({
            'enabled': True, 'dimensionality': '3d', 'kind': 'connection',
            'source': {'kind': 'fields', 'fields': {'x': 'src_x', 'y': 'src_y', 'z': 'src_z'}},
            'target': {'kind': 'fields', 'fields': {'x': 'tip_x', 'y': 'tip_y', 'z': 'tip_z'}},
    ...
    {
        'name': f'{_NNV}-3d', 'class_name': _NNV, 'dimensionality': '3d', 'enabled': True,
        'binding_json': json.dumps({
            'enabled': True, 'dimensionality': '3d', 'kind': 'connection',
            'source': {'kind': 'fields', 'fields': {'x': 'src_x', 'y': 'src_y', 'z': 'src_z'}},
            'target': {'kind': 'fields', 'fields': {'x': 'tip_x', 'y': 'tip_y', 'z': 'tip_z'}},
...
SEED_SIMULATION_DEFINITIONS.append({
    ...
    'participating_sim_state_classes_json': json.dumps([_NB, _NROD, _NGV, _NNV]),
...
SEED_PENDULUM_STEP_SOLUTION_DEFS.extend(
    [_NB_GRAV_DEF, _NB_INT_DEF, _NB_ROD_DEF, _NB_GVEC_DEF, _NB_NVEC_DEF]
)
SEED_PENDULUM_STEP_SOLUTIONS.extend(
    [_NB_GRAV_META, _NB_INT_META, _NB_ROD_META, _NB_GVEC_META, _NB_NVEC_META]
)
SEED_PENDULUM_STEP_EQUATIONS = list(SEED_PENDULUM_STEP_EQUATIONS) + (
    _NB_GRAV_EQ + _NB_INT_EQ + _NB_ROD_EQ + _NB_GVEC_EQ + _NB_NVEC_EQ
)
...
_NEWT_TENSION_BINDINGS = [
    _binding_sim_state('Tx', 'ftens_x', _NB, 'ftens_x'),
    _binding_sim_state('Ty', 'ftens_y', _NB, 'ftens_y'),
    _binding_sim_state('Tz', 'ftens_z', _NB, 'ftens_z'),
]
