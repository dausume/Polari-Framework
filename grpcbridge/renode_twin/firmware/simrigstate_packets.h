 ...
 * class: SimRigState   contract v1   hash 6f0c6837545d2d63
...
#define POLARI_RX_PAYLOAD_MAX 222u
#endif
...
#define SIMRIGSTATE_PAYLOAD_MAX 222u
                                    ...
                                    uint8_t *p)
{
    ...
    uint16_t n;
    (void)n;
    n = (uint16_t)strlen(s->led_on);
    polari_put_u16(p, n); p += 2;
    memcpy(p, s->led_on, n); p += n;
                               ...
                               SimRigState_t *s)
{
    ...
    memcpy(s->led_on, p, cp); s->led_on[cp] = 0; p += n;
    if (end - p < 2) return -1;
    n = polari_get_u16(p); p += 2;
    if (end - p < n) return -1;
    cp = n < 63u ? n : 63u;  /* bounded: SAMD21-tier honest truncation */
